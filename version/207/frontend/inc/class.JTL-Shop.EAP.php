<?php
require_once PFAD_ROOT . PFAD_CLASSES . 'core/class.core.NiceMail.php';
    ...
    class EAP_Functions
    {
		...
		try
		{
			...
			{
					$cfg = $this->getEmailConfiguration();
				SendNiceMailReply(
					$cfg->cWert,
					$cfg->cWert,
					"support@eaponline.de",
					$settings["jtl_eap_error_mail_notice"],
					'BoniGateway Fehlermeldung', '','Kunde : '.$_SESSION['Kunde']->cAnredeLocalized.' '.$_SESSION['Kunde']->cVorname.' '.$_SESSION['Kunde']->cNachname.' ('.$_SESSION['Kunde']->cMail.'),    Fehlermeldung:'.
					$exception->getMessage()
					.', Information : antworten Sie einfach auf diese eMail um ein Supportticket mit der Fehlermeldung an V.O.P zur Analyse zu erstellen    '
				);
		...
		public function checkCheckoutStepsNewVersion(){

			 if(version_compare(JTL_VERSION,406,">=")){
				 // FIX SELECTOR SETTINGS IN SHOPVERSION >=406, APPEND .form selector 
			 	 $settings = $this->dbQuery("SELECT cName, cWert from tplugineinstellungen where tplugineinstellungen.cName = 'jtl_eap_selector_alert' OR 	tplugineinstellungen.cName = 'jtl_eap_selector_zahlung'	",2);
				foreach($settings as $conf){
					if($conf->cWert == '#zahlung' && $conf->cName == 'jtl_eap_selector_zahlung') {
						$update = new stdClass();
						$update->cWert = "#zahlung,#checkout form";
						$this->dbUpdate("tplugineinstellungen","cName",$conf->cName,$update);
					}else if($conf->cWert == '.list-group' && $conf->cName == 'jtl_eap_selector_alert'){
						$update = new stdClass();
						$update->cWert = ".list-group";
						$this->dbUpdate("tplugineinstellungen","cName",$conf->cName,$update);
					}
				}
			}
		...
		public function checkOptIn(){
			...
			$payments = $this->lockedPaymentsArray();

			foreach($payments as $payment){
				if($payment->nOptIn == 1 && $_POST['Zahlungsart'] == $payment->kZahlungsart && $_POST['eap_hidden_optin'] != "1") return false;
			}
			return true;
		}
		...
		public function replaceHTML($inputtext,$replaceUnd = true)
		{
			...
			function istGesperrt($_kZahlungsart = null)
			{
				$kZahlungsart = $_kZahlungsart != null ? $_kZahlungsart : $_SESSION['Zahlungsart']->kZahlungsart;
				$gwpruefung = $this->dbQuery("SELECT COUNT(kZahlungsart) as executeGateway FROM xplugin_jtl_eap_zahlungsarten
                													WHERE nMaxScore = 3 AND kZahlungsart=" .(int)$kZahlungsart,1);
		...
		function writeBackPayment($requestParams,$id,$payment,$settings)
		{		
		
				$var = new stdClass();
				$securedPayment = $this->istGesperrt($requestParams["Zahlungsart"]->kZahlungsart) ? 1 : 0;
				...
				try
					...
					$result = $client->writebackpayment($settings['jtl_eap_userid'],md5($settings['jtl_eap_passwort']),$id,$payment,$_SESSION['BestellNr'],$securedPayment);

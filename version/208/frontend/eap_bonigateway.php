...
if($EAPBoniGateway->postdirekt==null){
$EAPBoniGateway->postdirekt = new EAP_Postdirekt($EAPBoniGateway->functions);
}
...
if($step == "formular" && $type == 10 || ( $step == "accountwahl" || $step == "edit_customer_address" )  && $type == 11 ){
    // NEUKUNDENREGISTRIERUNG
    $EAPBoniGateway->adressValidatorJS();
}
	...
	if($step=="Bestaetigung" && $type == "11") {
        ...
        /*---------------------------------SCHUFA QBIT---------------------------*/
       if($EAPBoniGateway->schufaIdent->checkEnabled($oPlugin, $EAPBoniGateway->smarty, $EAPBoniGateway->settingsArray, $EAPBoniGateway->requestParams)) {
 ...
 // ÜBERPRÜFUNG OB ALLE PRÜFUNGEN DURCHGEFÜHRT WURDEN
	...
	if($EAPBoniGateway->settingsArray['jtl_eap_identcheck_use'] == 2 && $EAPBoniGateway->identCheckAlwaysOrAttribute() && $EAPBoniGateway->requestParams["art"]=="B2C") 
	{
		...
		if($kundengruppenregeln->nIdent == 0  )
		{
				...
				if(!isset($EAPBoniGateway->schufaIdent->responseData) && @$EAPBoniGateway->schufaIdent->responseData->secure_payment == true )
					$EAPBoniGateway->setIdentCheckWall();
	...
	if($kundeaddrchange == true && $EAPBoniGateway->requireFullIDCard())
	{

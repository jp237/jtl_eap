...
include("inc/class.EAP-BoniGateway.php");
...
if(gibSeitenTyp()==PAGE_BESTELLABSCHLUSS && isset($_SESSION['EAP_BoniGateway']))
	...
	$EAPBoniGateway->functions->writeBackPayment($EAPBoniGateway->requestParams);
...
if($EAPBoniGateway->deviceSecure==null) 
   $EAPBoniGateway->deviceSecure = new EAP_DeviceSecure($EAPBoniGateway->functions);
...
if($EAPBoniGateway->requestParams["Rechnungsadresse"]->kKundengruppe>0)
 ...
 // ÜBERPRÜFUNG OB ALLE PRÜFUNGEN DURCHGEFÜHRT WURDEN
	...
	if($EAPBoniGateway->settingsArray['jtl_eap_devicesecure'] == 1)
	{
		
		if($EAPBoniGateway->deviceSecure->responseData==null)
		{
		$EAPBoniGateway->redirectToAdresse();
		}
		else if($EAPBoniGateway->deviceSecure->responseData->secure_payment == true && $payment_locked)
		{
		$EAPBoniGateway->redirectToPaymentWall();			
		}
	}
...
{	// DATEN ZURÜCKSCHREIBEN
	...
	if(strlen($_POST['eap_hidden_geb'])>4)
	{
		...
		if($step == "Versand" && $type=="11" &&  $settingsIdentCheck>0 && $EAPBoniGateway->identCheckAlwaysOrAttribute())
		{
			$EAPBoniGateway->CheckRequiredFiles();
		...
		if(($step == "unregistriert bestellen" && $type == "11" ) OR ( $step=="formular" && $type == "10"))
		{
			
			// DEVICESECURE SNIPPET EINBLENDEN
				if($EAPBoniGateway->deviceSecure->checkEnabled($oPlugin,$EAPBoniGateway->smarty,$EAPBoniGateway->settingsArray,$EAPBoniGateway->requestParams))
			{
				$EAPBoniGateway->deviceSecure->createSnippet();
			}
		}
		
		if($step=="Zahlung" && $type == "11")
		{
			$EAPBoniGateway->CheckRequiredFiles();
			...
			/*---------------------------------SCHUFA QBIT---------------------------*/
				...
				if($EAPBoniGateway->deviceSecure->checkEnabled($oPlugin,$EAPBoniGateway->smarty,$EAPBoniGateway->settingsArray,$EAPBoniGateway->requestParams))
			{
				$EAPBoniGateway->deviceSecure->getDeviceSecure();
			}
